...
The optimal sharing percentage for chess reinforcement learning is approximately 30-35\% of parameters, concentrated in middle residual layers. This contradicts the intuition that extensive sharing maximizes knowledge transfer, instead, sharing 86\% of parameters (E4) creates catastrophic optimization conflicts. The hierarchical knowledge transfer hypothesis suggests that low-level features require cluster-specific pattern recognition, mid-level features can share general tactical motifs, and high-level features must specialize for strategic evaluation.

Complete independence shows minimal behavioral separation despite substantial parameter-level divergence, revealing a critical distinction between parameter space differences and functional behavioral specialization. The no-sharing baseline (B2) exhibited policy head divergence (0.228) comparable to selective sharing configurations (E1-E4: 0.211-0.225), yet produced minimal behavioral differences (0.20\% vs 0.91-1.95\%). This demonstrates that architectural constraints from selective sharing actively transform parameter divergence into meaningful behavioral specialization. The finding challenges assumptions about how diversity arises in multi-agent systems, suggesting that optimization constraints from partial parameter sharing create functional specialization that data heterogeneity alone cannot produce.
