...
The evaluation system implements the metrics framework for assessing playstyle preservation and playing strength. The implementation spans multiple modules in \texttt{server/evaluation/} that orchestrate game play, analyze positions, classify moves, and compute model divergence.
...
The \texttt{PlaystyleMetricsCalculator} in \texttt{server/evaluation/playstyle\_metrics.py} implements the tactical score computation from Section~\ref{sec:evaluation-metrics}. The \texttt{GameAnalyzer} parses PGN strings, replays move sequences, and extracts features at each position using \texttt{python-chess} methods:
