#!/bin/bash
# Launch script for B2: No Sharing Baseline Experiment
# Clusters train independently with no inter-cluster aggregation
...
echo -e "${BLUE}================================================================${NC}"
echo -e "${BLUE}B2: No Sharing Baseline Experiment${NC}"
echo -e "${BLUE}================================================================${NC}"
echo ""

# Configuration
SERVER_CONFIG="chess-federated-learning/config/server_config_B2.yaml"
...
echo -e "${YELLOW}Aggregation: Intra-cluster ONLY (no inter-cluster sharing)${NC}"
echo ""
...
# Copy B2 config to server_config.yaml temporarily
...
uv run chess-federated-learning/server/run_training.py \
    ...
    > logs/server_B2.log 2>&1 &
SERVER_PID=$!

echo -e "${GREEN}Server started with PID: ${SERVER_PID}${NC}"
echo -e "${YELLOW}Server logs: logs/server_B2.log${NC}"
echo ""
...
if ! kill -0 $SERVER_PID 2>/dev/null; then
    echo -e "${RED}Error: Server failed to start. Check logs/server_B2.log${NC}"
    exit 1
fi
...
uv run chess-federated-learning/scripts/start_all_nodes.py \
    ...
    --server-port 8765 \
    --delay 0.5 \
    > logs/nodes_B2.log 2>&1 &
NODES_PID=$!

echo -e "${GREEN}Nodes started with PID: ${NODES_PID}${NC}"
echo -e "${YELLOW}Nodes logs: logs/nodes_B2.log${NC}"
echo ""
...
echo ""
echo -e "${BLUE}================================================================${NC}"
echo -e "${GREEN}B2 Experiment Setup Complete!${NC}"
echo -e "${BLUE}================================================================${NC}"
echo ""
...
echo -e "  - Experiment: B2 (No Sharing Baseline)"
echo -e "  - Rounds: 500"
echo -e "  - Games per node per round: 400"
echo -e "  - Total nodes: 8 (4 tactical + 4 positional)"
echo -e "  - Shared layers: NONE"
echo -e "  - Cluster-specific layers: ALL (complete independence)"
echo -e "  - Intra-cluster aggregation: YES (within each cluster)"
echo -e "  - Inter-cluster aggregation: NO (clusters never share)"
echo ""
echo -e "${YELLOW}To view logs:${NC}"
echo -e "  Server: tail -f logs/server_B2.log"
echo -e "  Nodes:  tail -f logs/nodes_B2.log"
echo ""
