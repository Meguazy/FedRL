#!/bin/bash
# Launch script for P2: Share Middle Layers Experiment
# Shares middle residual blocks (6-12) while keeping early/late layers independent
...
echo -e "${BLUE}================================================================${NC}"
echo -e "${BLUE}P2: Share Middle Layers Experiment${NC}"
echo -e "${BLUE}================================================================${NC}"
echo ""

# Configuration
SERVER_CONFIG="chess-federated-learning/config/server_config_P2.yaml"
...
echo -e "${YELLOW}Shared layers: Middle res_blocks (6-12)${NC}"
echo -e "${YELLOW}Cluster-specific: Input + early (0-5) + late (13-18) + heads${NC}"
echo ""
...
# Copy P2 config to server_config.yaml temporarily
...
uv run chess-federated-learning/server/run_training.py \
    ...
    > logs/server_P2.log 2>&1 &
SERVER_PID=$!

echo -e "${GREEN}Server started with PID: ${SERVER_PID}${NC}"
echo -e "${YELLOW}Server logs: logs/server_P2.log${NC}"
echo ""
...
if ! kill -0 $SERVER_PID 2>/dev/null; then
    echo -e "${RED}Error: Server failed to start. Check logs/server_P2.log${NC}"
    exit 1
fi
...
uv run chess-federated-learning/scripts/start_all_nodes.py \
    ...
    --server-port 8765 \
    --delay 0.5 \
    > logs/nodes_P2.log 2>&1 &
NODES_PID=$!

echo -e "${GREEN}Nodes started with PID: ${NODES_PID}${NC}"
echo -e "${YELLOW}Nodes logs: logs/nodes_P2.log${NC}"
echo ""
...
echo ""
echo -e "${BLUE}================================================================${NC}"
echo -e "${GREEN}P2 Experiment Setup Complete!${NC}"
echo -e "${BLUE}================================================================${NC}"
echo ""
...
echo -e "  - Experiment: P2 (Share Middle Layers)"
echo -e "  - Rounds: 500"
echo -e "  - Games per node per round: 400"
echo -e "  - Total nodes: 8 (4 tactical + 4 positional)"
echo -e "  - Shared layers: res_blocks 6-12 (7 blocks)"
echo -e "  - Cluster-specific layers: input + res_blocks 0-5, 13-18 + heads (12 blocks + 2 heads)"
echo -e "  - Intra-cluster aggregation: YES"
echo -e "  - Inter-cluster aggregation: YES (middle layers only)"
echo ""
echo -e "${YELLOW}To view logs:${NC}"
echo -e "  Server: tail -f logs/server_P2.log"
echo -e "  Nodes:  tail -f logs/nodes_P2.log"
echo ""
