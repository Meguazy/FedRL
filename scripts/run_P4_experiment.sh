#!/bin/bash
# Launch script for P4: Share Backbone, Separate Heads Experiment
# Shares entire backbone (input + all residual blocks 0-18) while keeping heads independent
...
echo -e "${BLUE}================================================================${NC}"
echo -e "${BLUE}P4: Share Backbone, Separate Heads Experiment${NC}"
echo -e "${BLUE}================================================================${NC}"
echo ""

# Configuration
SERVER_CONFIG="chess-federated-learning/config/server_config_P4.yaml"
...
echo -e "${YELLOW}Shared layers: Input + ALL res_blocks (0-18) - ENTIRE BACKBONE${NC}"
echo -e "${YELLOW}Cluster-specific: ONLY heads (policy + value)${NC}"
echo ""
...
# Copy P4 config to server_config.yaml temporarily
...
uv run chess-federated-learning/server/run_training.py \
    ...
    > logs/server_P4.log 2>&1 &
SERVER_PID=$!

echo -e "${GREEN}Server started with PID: ${SERVER_PID}${NC}"
echo -e "${YELLOW}Server logs: logs/server_P4.log${NC}"
echo ""
...
if ! kill -0 $SERVER_PID 2>/dev/null; then
    echo -e "${RED}Error: Server failed to start. Check logs/server_P4.log${NC}"
    exit 1
fi
...
uv run chess-federated-learning/scripts/start_all_nodes.py \
    ...
    --server-port 8765 \
    --delay 0.5 \
    > logs/nodes_P4.log 2>&1 &
NODES_PID=$!

echo -e "${GREEN}Nodes started with PID: ${NODES_PID}${NC}"
echo -e "${YELLOW}Nodes logs: logs/nodes_P4.log${NC}"
echo ""
...
echo ""
echo -e "${BLUE}================================================================${NC}"
echo -e "${GREEN}P4 Experiment Setup Complete!${NC}"
echo -e "${BLUE}================================================================${NC}"
echo ""
...
echo -e "  - Experiment: P4 (Share Backbone, Separate Heads)"
echo -e "  - Rounds: 500"
echo -e "  - Games per node per round: 400"
echo -e "  - Total nodes: 8 (4 tactical + 4 positional)"
echo -e "  - Shared layers: input + ALL res_blocks 0-18 (entire backbone)"
echo -e "  - Cluster-specific layers: ONLY heads (policy + value)"
echo -e "  - Intra-cluster aggregation: YES"
echo -e "  - Inter-cluster aggregation: YES (entire backbone)"
echo ""
echo -e "${YELLOW}To view logs:${NC}"
echo -e "  Server: tail -f logs/server_P4.log"
echo -e "  Nodes:  tail -f logs/nodes_P4.log"
echo ""
