#!/bin/bash
# Launch script for B1: Full Sharing Baseline Experiment
# This script starts the server and all client nodes for the B1 experiment
...
echo -e "${BLUE}================================================================${NC}"
echo -e "${BLUE}B1: Full Sharing Baseline Experiment${NC}"
echo -e "${BLUE}================================================================${NC}"
echo ""

# Configuration
SERVER_CONFIG="chess-federated-learning/config/server_config_B1.yaml"
...
# Copy B1 config to server_config.yaml temporarily
...
uv run chess-federated-learning/server/run_training.py \
    ...
    > logs/server_B1.log 2>&1 &
SERVER_PID=$!

echo -e "${GREEN}Server started with PID: ${SERVER_PID}${NC}"
echo -e "${YELLOW}Server logs: logs/server_B1.log${NC}"
echo ""
...
if ! kill -0 $SERVER_PID 2>/dev/null; then
    echo -e "${RED}Error: Server failed to start. Check logs/server_B1.log${NC}"
    exit 1
fi
...
uv run chess-federated-learning/scripts/start_all_nodes.py \
    ...
    --server-port 8765 \
    --delay 0.5 \
    > logs/nodes_B1.log 2>&1 &
NODES_PID=$!

echo -e "${GREEN}Nodes started with PID: ${NODES_PID}${NC}"
echo -e "${YELLOW}Nodes logs: logs/nodes_B1.log${NC}"
echo ""
...
echo ""
echo -e "${BLUE}================================================================${NC}"
echo -e "${GREEN}B1 Experiment Setup Complete!${NC}"
echo -e "${BLUE}================================================================${NC}"
echo ""
...
echo -e "  - Experiment: B1 (Full Sharing Baseline)"
echo -e "  - Rounds: 500"
echo -e "  - Games per node per round: 400"
echo -e "  - Total nodes: 8 (4 tactical + 4 positional)"
echo -e "  - Shared layers: ALL (input + residual blocks + heads)"
echo -e "  - Cluster-specific layers: NONE"
echo ""
echo -e "${YELLOW}To view logs:${NC}"
echo -e "  Server: tail -f logs/server_B1.log"
echo -e "  Nodes:  tail -f logs/nodes_B1.log"
echo ""
