...
class BoardEncoder:
    ...
    def _encode_piece_planes(self, board: chess.Board, history: Optional[List[chess.Board]] = None) -> np.ndarray:
        ...
        log.debug("Encoding piece planes with history...")
        planes = np.zeros((96, 8, 8), dtype=np.float32)
        
        # Prepare history (current + previous positions)
        if history is None:
            log.warning("No history provided, using current position for all history planes.")
            history = []
        ...
        log.debug(f"Padding history with current position. Padding nunmber: {self.history_length - len(boards)}")
        while len(boards) < self.history_length:
            log.debug("Padding history with current position.")
            boards.append(board)
        ...
        for i, hist_board in enumerate(boards):
            ...
            # Encode white pieces
            log.debug(f"Encoding history position {i} at offset {offset}.")
            ...
            log.debug(f"Encoded white pieces for history position {i}.")
                    
            # Encode black pieces
            log.debug(f"Encoding black pieces for history position {i}.")
            for piece_idx, piece_type in enumerate(self.PIECE_TYPES):
                ...
                for square in squares:
                    rank, file = divmod(square, 8)
                    planes[plane_idx, rank, file] = 1.0
            log.debug(f"Encoded black pieces for history position {i}.")
            
        log.success("All piece planes encoded.")
        return planes
    ...
    def _encode_repetition_planes(self, board: chess.Board) -> np.ndarray:
        ...
        log.debug("Encoding repetition planes...")
        planes = np.zeros((2, 8, 8), dtype=np.float32)
        
        # Check if current position is a repetition
        if board.is_repetition(2):
            log.debug("Current position is a repetition (1 time).")
            planes[0, :, :] = 1.0  # 1 repetition
        if board.is_repetition(3):
            log.debug("Current position is a repetition (2 or more times).")
    ...
    def _encode_castling_planes(self, board: chess.Board) -> np.ndarray:
        ...
        log.debug("Encoding castling rights...")
        planes = np.zeros((4, 8, 8), dtype=np.float32)
        
        if board.has_kingside_castling_rights(chess.WHITE):
            log.debug("White has kingside castling rights.")
            planes[0, :, :] = 1.0
        if board.has_queenside_castling_rights(chess.WHITE):
            log.debug("White has queenside castling rights.")
            planes[1, :, :] = 1.0
        if board.has_kingside_castling_rights(chess.BLACK):
            log.debug("Black has kingside castling rights.")
            planes[2, :, :] = 1.0
        if board.has_queenside_castling_rights(chess.BLACK):
            log.debug("Black has queenside castling rights.")
            planes[3, :, :] = 1.0
    ...
    def _encode_color(self, board: chess.Board) -> np.ndarray:
        ...
        log.debug("Encoding side to move...")
        plane = np.zeros((1, 8, 8), dtype=np.float32)

        if board.turn == chess.WHITE:
            log.debug("It's white's turn to move.")
            plane[0, :, :] = 1.0
        else:
            log.debug("It's black's turn to move.")
            plane[0, :, :] = 0.0
    ...
    def _encode_move_count(self, board: chess.Board) -> np.ndarray:
        ...
        plane[0, :, :] = normalized
        logger.debug(f"Move count plane encoded with value: {normalized}")
    ...
    def _encode_no_progress_count(self, board: chess.Board) -> np.ndarray:
        ...
        log.debug("Encoding no-progress count planes...")
